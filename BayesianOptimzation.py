...
class BayesianOptimizer:
    """Bayesian Optimization for kernel parameter tuning"""
    ...
    def update_with_result(self, parameters: Dict[str, float], score: float):
        ...
        if len(self.X_history) >= 2:
            X_normalized = np.array([self._normalize_parameters(x) for x in self.X_history])
    ...
    def optimize(self, objective_function: Callable[[Dict[str, float]], float]) -> OptimizationResult:
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                ...
                print(f"Iteration {iteration + 1}: Score = {score:.6f}, Params = {next_params}")
                ...
                if len(self.y_history) >= 3:
                    recent_improvement = max(self.y_history[-3:]) - max(self.y_history[-6:-3]) if len(self.y_history) >= 6 else float('inf')
                ...
                print(f"Error evaluating parameters at iteration {iteration + 1}: {e}")
                continue
    ...
    def get_posterior_statistics(self, test_points: Optional[List[Dict[str, float]]] = None) -> Dict:
        ...
        if test_points is None:
            ...
            test_points_array = [self._dict_to_parameters(tp) for tp in test_points]
        
        # Get predictions
        X_test_norm = np.array([self._normalize_parameters(x) for x in test_points_array])
